...
# - level: The logging level. Adjust to the desired level
...
#  ALL CONFIGURATIONS ARE DONE HERE AND WORK AS I EXPECT.
...
# DATABASE CONNECTION IS SET TO TRUE, ALL DATA HERE ARE CORRECT.
# Database connection
...
def get_old_email(user_id):
    ...
    # Fetch the first row of the result (assuming there is one) and get the email value
...
def register():
    ...
    if request.method == "POST":
        ...
        email_message.body = (
            ...
            f"this email. For any assistance or questions, feel free to contact our support team.\n\n"
            f"Best regards,\n"
            f"The Intuitivers Team"
        )
        mail.send(email_message)
...
def send_tfa_token_email(email, token, username):
    ...
    msg.html = (
        ...
        f"<p style='font-size: 30px; font-weight: bold;'>Verification Code: {token}</p>"
        ...
        f"<p>Best regards,<br>The Intuitivers Team</p>"
    )
    mail.send(msg)
...
def login():
    ...
    If the request method is POST, retrieve the
    entered username and password from the database.
    Check if the username exists in the database and
    verify the entered password to match with the stored password.
    If the username and password match with the stored user data
    AND If the user is verified and has 2FA enabled, Generate a token,
    send it via email, and proceed to 2FA verification process.

    Else:
        If 2FA is not enabled, proceed to the dashboard
         and store user data in the session. If the user is not verified,
         render an account not verified template to inform a user. If the
         username or password is incorrect, display a flash message for incorrect
         login credentials. If the request method is GET, render the login form again.

    Returns:
        If login credentials (username and or password)
         are incorrect, flash an invalid username or password message.
    ...
    if request.method == "POST":
        ...
        # check if the password during login match with the
        ...
        if user and check_password_hash(user[5], password):
            ...
            if user[12]:  # verified column is the 13th column in the table accounts
                ...
                if (
                        user[14] == "T"
                ):  # tfa column is the 15th column in the table accounts
                    ...
                    # Send the token to the user's email address and store it in database
            ...
            #  Extend the login process if username or password is incorrect.
...
def dashboard():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT username, profile_picture, role " "FROM accounts WHERE id = %s",
        (user_id,),
    )
    user = cursor.fetchone()
    cursor.close()
    ...
    if token_data:
        ...
        if request.method == "POST":
            ...
            email_message.html = (
                ...
                f"<p>Best regards</p>\n\n"
                f"<p>The Intuitivers Support Team</p>"
        ...
        if time_difference > 30:
            ...
            try:
                ...
                notification_email_body = (
                    "Hello,\n\nYour email address associated with your Intuitivers "
...
# THE ONLY ONE DIFFICULT IS HOW TO ALLOW USER TO GO BACK AFTER
# BAD REQUEST OR OTHER ERRORS: IN THE SPECIFIC ROUTE AND NOT TO GO TO HOME PAGE AGAIN
